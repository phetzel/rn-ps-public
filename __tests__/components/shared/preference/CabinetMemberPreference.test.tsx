...
const createMockPreference = (authorizedContent?: string): CabinetPreference => ({
  preference: {
...
describe('CabinetMemberPreference', () => {
  it('displays cabinet member name when preference is available', () => {
    const cabinetMember = createMockCabinetMember(CABINET_PREFERENCE_THRESHOLD + 10);
  ...
  it('shows preference content when relationship meets threshold', () => {
    const cabinetMember = createMockCabinetMember(CABINET_PREFERENCE_THRESHOLD + 5);
    const cabinetPreference = createMockPreference();
    ...
    expect(
      screen.getByText('This is the best approach based on current intelligence'),
    ).toBeTruthy();
  });
  ...
  it('displays authorized intel when available and authorized', () => {
    const cabinetMember = createMockCabinetMember(CABINET_AUTHORIZED_THRESHOLD + 10);
    const cabinetPreference = createMockPreference(
    ...
    expect(
      screen.getByText('Classified intelligence suggests this approach will have maximum impact.'),
    ).toBeTruthy();
  });
  ...
  it('handles edge case just below threshold', () => {
    const cabinetMember = createMockCabinetMember(CABINET_PREFERENCE_THRESHOLD - 1);
    const cabinetPreference = createMockPreference();
  ...
  it('handles different answer types', () => {
    ...
    expect(screen.getByText('We need to be more aggressive on this issue')).toBeTruthy();
  });
