...
jest.mock(
  "~/components/shared/situations-outcome-list/SituationsOutcomeList",
  () => ({
    __esModule: true,
...
jest.mock(
  "~/components/shared/level-media-coverage/LevelMediaCoverage",
  () => ({
    __esModule: true,
    default: ({ levelId }: { levelId: string }) => {
      const { Text } = require("react-native");
...
describe("LevelOverviewEvents", () => {
  ...
  it("passes levelId to both child components", () => {
    ...
    expect(
      screen.getByText("Situations Outcome List - custom-level-123")
    ).toBeTruthy();
    expect(
      screen.getByText("Level Media Coverage - custom-level-123")
    ).toBeTruthy();
  });
  ...
  it("renders both components in the correct order", () => {
    ...
    const situationsText = screen.getByText(
      "Situations Outcome List - level-1"
    );
