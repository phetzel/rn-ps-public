...
jest.mock("@nozbe/watermelondb/react", () => ({
  withObservables: jest.fn(
    (observablesGetter, observablesCreator) =>
...
jest.mock(
  "~/components/shared/exchanges-outcome-list/ExchangeOutcomeItem",
  () => {
...
describe("ExchangesOutcomeList", () => {
  ...
  beforeEach(() => {
    (observePressExchangesForLevel as jest.Mock).mockReturnValue(
      of(mockPressExchanges)
    );
  });
  ...
  const renderWithLevelId = (
    levelId = "level-1",
    pressExchanges = mockPressExchanges
  ) => {
    render(
      <ExchangesOutcomeList levelId={levelId} pressExchanges={pressExchanges} />
    );
  };
  ...
  it("has correct accessibility properties for accordion", () => {
    ...
    const accordion = screen.getByLabelText(
      "Expandable list of press exchanges"
    );
  ...
  it("renders empty state when no press exchanges", () => {
    ...
    expect(
      screen.getByText("No press exchanges found for this level.")
    ).toBeTruthy();
  ...
  it("passes correct levelId to observable", () => {
    ...
    expect(observePressExchangesForLevel).toHaveBeenCalledWith(
      "test-level-123"
    );
  });
});
