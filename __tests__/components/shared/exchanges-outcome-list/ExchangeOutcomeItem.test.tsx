...
describe('ExchangeOutcomeItem', () => {
  ...
  const renderWithExchange = (
    ...
    render(
      <ExchangeOutcomeItem exchange={exchange} situation={situation} journalist={journalist} />,
    );
  };
  ...
  it('has correct accessibility properties for accordion', () => {
    renderWithExchange();
    const accordion = screen.getByLabelText('Exchange with Sarah Chen. 1 questions answered.');
    expect(accordion).toBeTruthy();
  });

  it('has correct accessibility properties for accordion trigger', () => {
    renderWithExchange();
    const trigger = screen.getByLabelText('Expand to view exchange details with Sarah Chen');
  ...
  it('handles journalist not called on', () => {
    ...
    expect(accordion).toBeTruthy();
    const trigger = screen.getByLabelText('View exchange details with Sarah Chen');
    expect(trigger).toBeTruthy();
  });
  ...
  it('handles exchange with pending follow-up', () => {
    ...
    const content = screen.getByLabelText('1 questions and answers with one unanswered follow-up');
    expect(content).toBeTruthy();
  });
  ...
  it('handles exchange with multiple history items', () => {
    ...
    const accordion = screen.getByLabelText('Exchange with Sarah Chen. 2 questions answered.');
    expect(accordion).toBeTruthy();
    const content = screen.getByLabelText('2 questions and answers with all questions addressed');
    expect(content).toBeTruthy();
  });
