...
describe('SituationStatusBadge', () => {
  ...
  it('renders domestic policy badge', () => {
    ...
    expect(screen.getByLabelText('Situation type: Domestic Policy')).toBeTruthy();
  });
  ...
  it('renders foreign affairs badge', () => {
    ...
    expect(screen.getByLabelText('Situation type: Foreign Affairs')).toBeTruthy();
  });
  ...
  describe('badge variant assignment', () => {
    it('assigns default variant for domestic policy', () => {
      const { unmount } = render(<SituationStatusBadge status={SituationType.DomesticPolicy} />);
    ...
    it('assigns secondary variant for foreign affairs', () => {
      const { unmount } = render(<SituationStatusBadge status={SituationType.ForeignAffairs} />);
    ...
    it('assigns destructive variant for security', () => {
      const { unmount } = render(<SituationStatusBadge status={SituationType.Security} />);
    ...
    it('assigns destructive variant for ethics', () => {
      const { unmount } = render(<SituationStatusBadge status={SituationType.Ethics} />);
      // Component renders without error with destructive variant
      expect(screen.getByText('Ethics')).toBeTruthy();
      unmount();
    });
  ...
  describe('text formatting', () => {
    ...
    it('handles all enum values formatting', () => {
      Object.values(SituationType).forEach((type) => {
        ...
        const formattedText = type.replace('_', ' ').replace(/\b\w/g, (l) => l.toUpperCase());
