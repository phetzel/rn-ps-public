...
describe('SubgroupCategoryIcon', () => {
  it('renders political category icon', () => {
    const { unmount } = render(<SubgroupCategoryIcon category={SubgroupCategory.Political} />);
    expect(screen.getAllByLabelText('Political voter groups')[0]).toBeTruthy();
    unmount();
  });

  it('renders demographic category icon', () => {
    const { unmount } = render(<SubgroupCategoryIcon category={SubgroupCategory.Demographic} />);
    expect(screen.getAllByLabelText('Demographic voter groups')[0]).toBeTruthy();
    unmount();
  });

  it('renders economic category icon', () => {
    const { unmount } = render(<SubgroupCategoryIcon category={SubgroupCategory.Economic} />);
  ...
  it('renders with custom className', () => {
    const { unmount } = render(
      <SubgroupCategoryIcon category={SubgroupCategory.Political} className="text-red-500" />,
    );
  ...
  describe('handles all SubgroupCategory enum values', () => {
    it('renders all category types correctly', () => {
      Object.values(SubgroupCategory).forEach((category) => {
        const { unmount } = render(<SubgroupCategoryIcon category={category} />);
  ...
  describe('styling', () => {
    ...
    it('applies custom className correctly', () => {
      const customClasses = ['text-red-500', 'text-blue-600', 'text-green-400', 'text-purple-700'];

      customClasses.forEach((className) => {
        const { unmount } = render(
          <SubgroupCategoryIcon category={SubgroupCategory.Economic} className={className} />,
        );
        expect(screen.getAllByLabelText('Economic voter groups')[0]).toBeTruthy();
        unmount();
      });
  ...
  describe('edge cases', () => {
    it('handles invalid category gracefully', () => {
      ...
      const { unmount } = render(<SubgroupCategoryIcon category={'invalid' as SubgroupCategory} />);
