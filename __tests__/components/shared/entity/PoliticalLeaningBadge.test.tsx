...
describe("PoliticalLeaningBadge", () => {
  ...
  it("renders conservative leaning badge", () => {
    ...
    expect(
      screen.getByLabelText("Political leaning: Conservative")
    ).toBeTruthy();
  });
    ...
    it("renders all political leaning types correctly", () => {
      Object.values(PoliticalLeaning).forEach((leaning) => {
        const { unmount } = render(
          <PoliticalLeaningBadge politicalLeaning={leaning} />
        );

        const formattedLeaning =
          leaning.charAt(0).toUpperCase() + leaning.slice(1);
        expect(screen.getByText(formattedLeaning)).toBeTruthy();
        expect(
          screen.getByLabelText(`Political leaning: ${formattedLeaning}`)
        ).toBeTruthy();
        unmount();
      });
    });
