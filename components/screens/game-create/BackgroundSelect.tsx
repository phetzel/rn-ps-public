...
export function BackgroundSelect({ control, error, disabled = false }: Props) {
  ...
  const contentInsets = { top: 0, bottom: Platform.select({ ios: 0, android: 24 }), left: 12, right: 12 };
        ...
        <View className="gap-2">
          ...
          >
            <View onLayout={(e) => setTriggerWidth(e.nativeEvent.layout.width)}>
              <SelectTrigger className="h-12 w-full" aria-labelledby="poBackgroundLabel" aria-describedby={error ? errorId : undefined}>
                {value ? (
            ...
            <SelectContent side="top" insets={contentInsets} style={triggerWidth ? { width: triggerWidth } : undefined}>
              <SelectGroup>
                {options.map((opt) => (
         ...
         {error && (
            <Text nativeID={errorId} accessibilityRole="alert" className="text-destructive mt-1 text-sm">
              {error.message}
            </Text>
          )}
        </View>
