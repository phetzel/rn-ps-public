...
const ConsequenceGameComplete = ({ game, presApprovalRating }: ConsequenceGameCompleteProps) => {
  if (!game) {
    return null;
  }
    ...
    >
      ...
      >
        ...
        <Text className="text-sm text-green-700 mb-3">
          Congratulations, {game.psName}! You successfully served as Press Secretary for the full
          4-year term under President {game.presName}.
        </Text>
      ...
      >
        ...
        <View className="gap-3" accessible={false}>
          ...
          <View className="flex-row justify-between items-center">
            ...
            <Text
              className={`text-sm font-bold ${
                game.presPsRelationship >= 50 ? 'text-green-600' : 'text-orange-600'
              }`}
            >
