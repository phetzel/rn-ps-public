...
export function QuestionDisplay({ question, isFollowUpQuestion = false }: QuestionDisplayProps) {
  return (
    <View
      accessible={true}
      accessibilityLabel={`${isFollowUpQuestion ? 'Follow-up question' : 'Question'}: ${question}`}
    >
      {isFollowUpQuestion && <FollowUpBadge />}

      <View className="flex-row items-center gap-2" accessible={false}>
        <View className="bg-primary/10 rounded-full p-2">
          <MessageSquare className="text-primary" accessibilityLabel="Question icon" />
        </View>
