...
- [ ] Add Sentry (Expo) and initialize early
  - [ ] Install
    ```bash
    npx expo install @sentry/react-native sentry-expo
    ```
  - [ ] Configure plugin in `app.json` (or move to `app.config.ts`)
    ```json
    {
      "expo": {
        "plugins": [
    ...
    ```
  - [ ] Add DSN via EAS Secrets: `SENTRY_DSN`
  - [ ] Initialize Sentry in the app entry before rendering
  - [ ] Mask PII in breadcrumbs and events
- [ ] Add a global error boundary UI (expo-router `app/+error.tsx`)
  - [ ] Show user-friendly fallback, a reset action, and send report to Sentry
...
- [ ] Migrate to `app.config.ts`
  - [ ] Centralize `expo.extra` runtime flags and environment
  - [ ] Read secrets from EAS Secrets (Sentry, analytics, AdMob IDs)
- [ ] Ensure no secrets in repo; add `.env*` to gitignore if needed
...
- [ ] ESLint + Prettier + import sort
  ...
  ```
- [ ] Add ESLint config (strict, RN, hooks, import/order)
- [ ] Prettier config and `.editorconfig`
- [ ] Husky + lint-staged for pre-commit
  ```bash
  npx husky init
  npm i -D lint-staged
  ```
- [ ] Update CI to enforce typecheck, lint, tests
...
- [ ] ESLint/Prettier/Husky + CI enforcement
...
jobs:
  build:
    ...
    steps:
      ...
      - run: npm run test:ci
      - run: npm run lint || echo "lint job pending" # add once configured
