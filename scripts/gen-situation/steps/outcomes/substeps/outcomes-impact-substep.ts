...
export class OutcomesImpactsSubstep extends ResponsesGenerationStep<
  ImpactsInput,
  GenerateOutcomesConsequences
> {
  ...
  protected validateInput(input: ImpactsInput): void {
    ...
    if (!input?.baseOutcomes?.outcomes?.length)
      throw new Error('Impacts sub-step requires base outcomes');
  }
  ...
  protected async transform(
    ...
    const byId = new Map(
      parsed.outcomeConsequences.map((o) => [o.outcomeId, o.consequences] as const),
    );
